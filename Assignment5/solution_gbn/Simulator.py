...
class Simulator:
    def __init__(self, options, TRACE, cbA = None, cbB = None):
        ...
        self.entity_A             = EntityA(self, self.seqnum_limit, TRACE)
        self.entity_B             = EntityB(self, self.seqnum_limit)
        self.event_list           = []
  ...
  Call ignored.''')
    ...
    def stop_timer(self, entity):
        ...
        i = 0
        while i < len(self.event_list):
            if self.event_list[i].ev_type == EventType.TIMER_INTERRUPT and self.event_list[i].ev_entity is entity:
                break
